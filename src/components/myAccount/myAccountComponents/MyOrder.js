    ...
    return (
        <div className="col-12 col-md-12 col-lg-8 col-xl-8 text-center">
			...
			{data && data.map((single)=>{
				return (
					<div key={single.id} className="ord_list_wrap border mb-4 mfliud">
						<div className="ord_list_head gray d-flex align-items-center justify-content-between px-3 py-3">
							<div className="olh_flex">
								...
								<h6 className="mb-0 ft-medium">#00453{single.id}</h6>
							</div>	
							<div className="olh_flex">
						...
						<div className="ord_list_body text-left">
							{/* <!-- First Product --> */}
							{single.products && single.products.map((product) =>{
								return (
									...
									<div className="row align-items-center justify-content-center m-0 py-4 br-bottom">
										<div className="col-xl-5 col-lg-5 col-md-5 col-12">
											<div className="cart_single d-flex align-items-start mfliud-bot">
												<div className="cart_selected_single_thumb">
													<a href="#"><img src={product.product.images[0].image} width="75" className="img-fluid rounded" alt="" /></a>
												</div>
												<div className="cart_single_caption pl-3">
													<p className="mb-0"><span className="text-muted small">{product.product.category}</span></p>
													<h4 className="product_title fs-sm ft-medium mb-1 lh-1">{product.product.product_name}</h4>
													<p className="mb-2"><span className="text-dark medium">Size: {product.size}</span>, <span className="text-dark medium">Count: {product.count}</span></p>
													<h4 className="fs-sm ft-bold mb-0 lh-1">₹{product.product.offer_price}</h4>
												</div>
											</div>
										</div>
										<div className="col-xl-3 col-lg-3 col-md-3 col-6">
											...
											<div className="delv_status">
												{single.isPaid?
												<span className="ft-medium small text-warning bg-light-warning rounded px-3 py-1">In Progress</span>
												:
												<div className="delv_status"><span className="ft-medium small text-danger bg-light-danger rounded px-3 py-1">Payment pending</span></div>
												}
											</div>
										</div>
										<div className="col-xl-4 col-lg-4 col-md-4 col-6">
											<p className="mb-1 p-0"><span className="text-muted">Expected date by:</span></p>
											<h6 className="mb-0 ft-medium fs-sm">{single.expected_date}</h6>
										</div>
									</div>
								)
							})}
						...
						<div className="ord_list_footer d-flex align-items-center justify-content-between br-top px-3">
							...
							<div className="col-xl-9 col-lg-9 col-md-8 pr-0 py-2 olf_flex d-flex align-items-center justify-content-between">
								...
								<div className="olf_inner_right"><h5 className="mb-0 fs-sm ft-bold">Total: ₹{single.order_amount}</h5></div>
							</div>
						</div>
					</div>
